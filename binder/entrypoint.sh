...
import_workspace() {
    ...
    # Imported under the name JupyterLab opens on a plain /lab URL. Under its
    # own name the layout is only reachable by typing that name, and visitors
    # get the base image's layout instead, which points at a notebook this
    # repository does not have.
    jupyter lab workspaces import --name default "${workspace_file}" \
        >/tmp/jupyter-workspace-import.log 2>&1 || \
